...
class BaseParser(object):
    ...
    def _call_api(self):
        ...
        if nsended != len(self.send_pkg):
            ...
            cur_pkg_idx = 0
            body_buf = bytearray()
            while cur_pkg_idx < self.pkg_num:
                ...
                if len(head_buf) == self.rsp_header_len:
                    body_buf_tmp = self.recv_one_pkg(head_buf)
                    cur_pkg_idx += 1
                    body_buf.extend(body_buf_tmp)
                else:
                    log.debug("head_buf is not 0x10")
                    raise ResponseHeaderRecvFails("head_buf is not 0x10 : " + str(head_buf))
            dd = self.parseResponse(body_buf)
            log.debug("pkg_num={} data={}".format(self.pkg_num, dd))
            return dd

    def recv_one_pkg(self, head_buf):

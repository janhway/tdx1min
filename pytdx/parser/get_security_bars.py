...
class GetSecurityBarsXCmd(BaseParser):

    def setParams(self, category, market_code_list, start, count):
        self.category = category
        self.market_code_list = market_code_list
        self.pkg_num = len(market_code_list)

        tmp_market_code_list = [(market, code.encode("utf-8")) for market, code in market_code_list]
        pkg_buf = bytearray()
        for market, code in tmp_market_code_list:
            values = (
                0x10c,
                0x01016408,
                0x1c,
            ...
            pkg = struct.pack("<HIHHHH6sHHHHIIH", *values)
            pkg_buf.extend(pkg)
        log.debug("pkg_num={} len(pkg)={}".format(self.pkg_num, len(pkg_buf)))
        self.send_pkg = pkg_buf

    def parseResponse(self, body_buf):
        pos = 0
        klines_map = {}
        for market, code in self.market_code_list:
            tmp_pos, klines = self.parseOneResponse(body_buf[pos:])
            pos += tmp_pos
            klines_map[code] = klines
        return klines_map

    def parseOneResponse(self, body_buf):
        pos = 0
        ...
        log.debug("len(body_buf)={} pos={}".format(len(body_buf), pos))
        return pos, klines

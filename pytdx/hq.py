...
from pytdx.parser.get_security_bars import GetSecurityBarsCmd, GetSecurityBarsXCmd
...
class TdxHq_API(BaseSocketClient):
    ...
    @update_last_ack_time
    def get_security_bars_x(self, category, market_code_list, start, count):
        cmd = GetSecurityBarsXCmd(self.client, lock=self.lock)
        cmd.setParams(category, market_code_list, start, count)
        return cmd.call_api()
